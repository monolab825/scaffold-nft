...
import { AnyLink, CARMetadata, UploadProgress, UploadStatus, Uploader, useUploader } from "@w3ui/react";
...
const Done = ({ file, files, dataCID, storedDAGShards }: DoneProps): ReactNode => {
  ...
  return (
    <div>
      ...
      <p>
        <a href={`https://w3s.link/ipfs/${cidString}`} className="text-blue-800">
...
function UploaderConsole(): ReactNode {
  ...
  switch (status) {
    ...
    case UploadStatus.Succeeded: {
      return <Done file={file} files={files} dataCID={dataCID} storedDAGShards={storedDAGShards} />;
    }
        ...
        <div className="flex flex-row space-x-2 flex-wrap max-w-xl">
          {files?.map((f, i) => (
            <div className="flex flex-col justify-around" key={i}>
              ...
              <span className="text-xs text-white/75 font-mono">{humanFileSize(f.size)}</span>
            </div>
          ))}
        </div>
        <div className="p-4">
